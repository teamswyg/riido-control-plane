...
func fixtureDockerfile(finalUser string) string {
	return strings.Join([]string{
		...
		"ENV CGO_ENABLED=0",
		`RUN --mount=type=cache,target=/go/pkg/mod --mount=type=cache,target=/root/.cache/go-build go build -trimpath -ldflags="-s -w" -o /out/riido_ai_server ./cmd/riido_ai_server`,
		"FROM scratch",
