...
func renderClaimSurfaces(b *strings.Builder, surfaces []claimSurface) {
	...
	fmt.Fprintln(b)
	fmt.Fprintln(b, "| Claim | Code | Test | Manifest | Generated docs | Observes covered | Verifies covered | Verifiers | Commands | Evidence chains |")
	fmt.Fprintln(b, "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |")
	for _, surface := range surfaces {
		fmt.Fprintf(b, "| `%s` | `%d` | `%d` | `%d` | `%d` | `%d` | `%d` | `%d` | `%d` | `%d` |\n",
			surface.ID, len(surface.CodePaths), len(surface.TestPaths),
			len(surface.ManifestPaths), len(surface.GeneratedDocs), len(surface.CoversObserves), len(surface.CoversVerifies), len(surface.Verifiers),
