...
func TestStoreMetricsSnapshotOptimizationEvidence(t *testing.T) {
	path := "../../docs/30-architecture/evidence/control-plane-store-metrics-snapshot-optimization-2026-07-03.json"
	...
	if evidence.Change.File != "internal/riidoaiserver/store_operation_metrics_snapshot.go" ||
	...
	if evidence.Improvement.MedianLatencyReductionPercent < 35 ||
		evidence.Improvement.BytesPerOpReductionPercent < 80 ||
		evidence.Improvement.AllocsPerOpReductionPercent < 50 {
