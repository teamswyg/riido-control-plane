...
func TestRunWritesEvidence(t *testing.T) {
	...
	if got.SchemaVersion != evidenceSchema || got.OwnedContexts != len(requiredOwnedContexts) {
		t.Fatalf("unexpected evidence: %+v", got)
	}
	if got.ForbiddenImports != 0 || got.Loop.Observation == "" {
		t.Fatalf("missing boundary evidence: %+v", got)
	}
}
