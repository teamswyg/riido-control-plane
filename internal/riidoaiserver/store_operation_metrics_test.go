...
func TestStoreOperationMetricsApplyToSnapshot(t *testing.T) {
	...
	if snapshot.StoreOperations[0].Operation != StoreOperationAppendEvent.String() ||
		snapshot.StoreOperations[1].Operation != StoreOperationPollAssignment.String() {
		t.Fatalf("store operations should be sorted by vocabulary key: %+v", snapshot.StoreOperations)
	}
	poll := snapshot.StoreOperations[1]
