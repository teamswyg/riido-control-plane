...
func assertCloudWatchEMFMetricUnits(t *testing.T, envelope cloudWatchEMFEnvelope) {
	...
	if metricUnits["sse_streams_active"] != "Count" ||
		metricUnits["sse_stream_ttfb_max_ms"] != "Milliseconds" ||
		metricUnits["sse_stream_duration_max_ms"] != "Milliseconds" {
		t.Fatalf("emf SSE stream metric units = %+v", metricUnits)
	}
