...
func TestDevelopmentAIAgentClientStoreKeepsProgressMessageAcrossProviderHeartbeat(t *testing.T) {
	...
	if got, want := threads.Threads[0].Message, "Rust 프로젝트 생성 실행 중 - Cargo 바이너리 프로젝트를 초기화합니다."; got != want {
...
func TestDevelopmentAIAgentClientStoreStripsPersistedRiidoLogBlocksFromThreadMessage(t *testing.T) {
	...
	if strings.Contains(threads.Threads[0].Message, "<riido_log>") || strings.Contains(threads.Threads[0].Message, `"code":`) {
		t.Fatalf("thread message leaked internal riido_log transport: %q", threads.Threads[0].Message)
	}
	if got, want := threads.Threads[0].Message, "완료했습니다.\n\n- `go test ./...` 통과"; got != want {
		t.Fatalf("thread message = %q, want %q", got, want)
	}
}
