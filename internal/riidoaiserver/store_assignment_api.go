...
func (s *Store) AssignTaskReplacement(ctx context.Context, taskID string, req AssignRequest) (assignment Assignment, err error) {
	ctx, span := s.startStoreOperationTrace(ctx, StoreOperationCreateAssignment)
	startedAt := time.Now()
	defer func() {
		s.observeStoreOperation(StoreOperationCreateAssignment, startedAt, err)
	...
	reply := make(chan assignResult, 1)
	cmd := assignCmd{taskID: taskID, req: req, forceNewAssignment: true, reply: reply}
