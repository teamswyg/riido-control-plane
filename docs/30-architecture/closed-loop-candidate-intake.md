...
| `operational-readiness` | `operational-readiness-closed-loop-candidates` | `closed_loop_candidate` | `6` |
...
- candidate intake evidence must expose candidate_subjects for every consumed candidate item that carries a subject
...
- candidate intake must reject consumed candidate artifacts unless source_generated_at < source_expires_at and now < source_expires_at
...
- candidate intake sources must include operational readiness stale partial candidates
