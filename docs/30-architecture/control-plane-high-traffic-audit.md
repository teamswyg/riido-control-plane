...
| `client_event_subscriber_fanout` | `in_memory_fanout` | `10` | `sync.Mutex=1, Lock()=2, make(chan=1, chan =2, select {=2, map[=10, append(=6` | Measure subscriber fanout pressure and terminal compensation counts before changing buffer size or adding richer realtime event types. |
