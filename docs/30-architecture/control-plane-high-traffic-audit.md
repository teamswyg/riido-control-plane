...
| `client_event_subscriber_fanout` | `in_memory_fanout` | `9` | `sync.Mutex=1, Lock()=2, make(chan=1, chan =2, select {=1, map[=8, append(=4` | Measure subscriber fanout pressure before adding richer realtime event types or per-message metadata. |
