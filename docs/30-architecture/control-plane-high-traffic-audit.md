...
- local pressure pprof: `go run ./tools/controlplanepressure -duration 5s -scenario http_endpoint_threads_v3 -concurrency 1,8,32,128 -threads 48 -lines 80 -pprof-dir out/control-plane-local-pprof -candidate-out out/control-plane-pressure-closed-loop-candidates-pprof.json -evidence-out out/control-plane-local-pressure-pprof.json`
...
| `client_event_subscriber_fanout` | `in_memory_fanout` | `10` | `sync.Mutex=1, Lock()=2, make(chan=1, chan =2, select {=2, map[=12, append(=6` | Measure subscriber fanout pressure and terminal compensation counts before changing buffer size or adding richer realtime event types. |
...
| `tool_approval_waiters` | `busy_worker_polling` | `8` | `make(chan=7, chan =19, select {=8, time.NewTimer=2, time.NewTicker=1, map[=1` | Measure approval waiter timeout pressure and keep web approval waits bounded by long-poll limits. |
