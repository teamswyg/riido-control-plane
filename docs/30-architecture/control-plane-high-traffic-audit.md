...
| `tool_approval_waiters` | `busy_worker_polling` | `4` | `make(chan=7, chan =19, select {=8, time.NewTimer=2, time.NewTicker=1, map[=1, append(=1` | Measure approval waiter timeout pressure and keep web approval waits bounded by long-poll limits. |
| `thread_history_v3_projection` | `read_model_projection` | `3` | `Lock()=1, map[=1, append(=2` | Measure per-thread message copy cost, keep agent snapshots by id, and cap projection work before adding richer timelines. |
