...
| `client_event_subscriber_fanout` | `in_memory_fanout` | `6` | `sync.Mutex=1, Lock()=2, make(chan=1, chan =2, select {=1, map[=7, append(=4` | Measure subscriber fanout pressure before adding richer realtime event types or per-message metadata. |
...
| `progress_ingest` | `runtime_progress_ingest` | `6` | `Lock()=1, map[=6, append(=4` | Batch tiny provider fragments into line or frame sized updates while preserving v2/v3 response shapes. |
