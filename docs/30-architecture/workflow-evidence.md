...
- workflow count: `56`
- evidence covered: `56`
- evidence tools covered: `49/49`
- evidence tools bound to artifacts: `49/49`
- accepted gaps: `0`
...
| Hypothesis | A workflow evidence manifest can make CI observability debt explicit, prove that each evidence-out file is bound to a fail-closed artifact upload, ensure evidence-capable tools are exercised by a declared execution route in evidence-producing mode, cover every GitHub Actions workflow extension, admit only attested private riido-ci routes with redacted always-collected artifacts, reject action versions known to emit runtime deprecation annotations, and carry a self-expiring freshness window refreshed by a scheduled workflow. |
| Execute | Scan .yml and .yaml GitHub Actions workflows, classify evidence coverage, require any executable workflow gap to be registered, require artifact upload steps to fail closed, load explicitly declared riido-ci pipeline files and fail closed unless they are active, private, attested, redacted and always collected, require every evidence-out path to be uploaded, compare evidence-out capable tools with declared execution-route invocations, require tool-level evidence-out bindings, publish generated_at/expires_at/evidence_ttl_hours/workflow_scheduled/strict_artifact_upload, require the workflow evidence workflow to run on a schedule with strict artifact upload, and fail on deprecated action references. |
...
- every evidence-out capable tool must be invoked by at least one GitHub Actions workflow or declared riido-ci pipeline
- declared riido-ci evidence routes must be active, private, attested, redacted, and always collected
