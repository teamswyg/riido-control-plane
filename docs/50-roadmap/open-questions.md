...
| Q-CP-007 | Runtime waitlist mutation | Should the Windows app waitlist, launch-notification, and marketing-consent action from Figma `node-id=275-22731` and web onboarding `node-id=236-29749` be implemented in the AI Agent control-plane client API or delegated to an existing product/user-marketing API? | Resolved as no-diff for the AI Agent control-plane client API. The generated client exposes only `GET /v1/client/ai-agent/devices` for runtime empty-state composition; Windows waitlist, launch notification, and marketing-consent actions remain client/product facts unless a future product/marketing SSOT defines a separate generated surface. This server must not expose AI Agent waitlist or marketing-consent endpoints. |
