...
| Q-CP-007 | Runtime waitlist mutation | Should the Windows app waitlist, launch-notification, and marketing-consent action from Figma `node-id=275-22731` and web onboarding `node-id=236-29749` be implemented in the AI Agent control-plane client API or delegated to an existing product/user-marketing API? | Unresolved. Current generated client exposes only `GET /v1/client/ai-agent/devices` for runtime empty-state composition; no waitlist/marketing mutation is generated until the owning SSOT is chosen. |
