...
| `queued-status-current-only` | comment_kind=queued_by_busy_agent means the durable assignment is waiting behind busy work. Client-visible projections must not render it as an Agent reply or thinking/progress row: hide queued work_status, comment_kind, message, result_message, queue diagnostics, and queued work-status SSE events. Preserve assignment_state=queued and the active stream so clients retain the non-terminal participant identity until a durable terminal transition. |
